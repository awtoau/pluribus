...
-----
  ...
  TRELLIS_DBROOT=... PYTHONPATH=... python3 load.py \
    --board boards/<name> \
    --label <LABEL> \
    --config /path/to/bitstream.bin.config
...
Or let scripts/run_pipeline.py drive the whole pipeline from board.toml.
"""
...
def dump_pins(config_path, annotations_path, out_path, device,
    ...
    preserved.  The output is in the standard pins.tsv column format so it
    can be diffed directly against an existing file.

    device/package/board_name are stamped into the header metadata that
    read_pins() parses back, so they must describe the board actually
    being dumped — never a hardcoded one.
    ...
    # Write TSV.  The device/package lines are metadata read_pins() parses
    # back, so they must reflect the board being dumped.
    title = f" — {board_name}" if board_name else ""
    header = (
        f"# Pluribus pin annotation file{title}\n"
        "#\n"
...
def main():
    ...
    if args.dump_pins:
        dump_pins(args.config, args.annotations, args.dump_pins, device,
                  package=package, board_name=board_cfg.get("name"))
        return
