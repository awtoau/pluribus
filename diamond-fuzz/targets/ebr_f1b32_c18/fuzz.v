// ebr_f1b32_18 — Gap 2 (#62): probe F1B32 positional vs config-dependent.
// Identical DP8KC placed at EBR_R6C18 via LPF LOCATE; F1B32 expected
// to be set at C15 only (positional-fixed per corpus evidence).
module fuzz (
    input  wire clk,
    output wire out0
);
...
DP8KC #(
    ...
    .CLKA(clk), .CEA(gnd), .OCEA(gnd), .WEA(gnd),
    .CSA2(gnd),.CSA1(gnd),.CSA0(gnd),.RSTA(gnd),
    .CLKB(clk), .CEB(gnd), .OCEB(gnd), .WEB(gnd),
    .CSB2(gnd),.CSB1(gnd),.CSB0(gnd),.RSTB(gnd),
